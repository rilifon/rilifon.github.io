...
class Navbar extends FlxBasic {
	...
	public function new(y : Float) {
		...
		buttons.add(new NavButton(50, y, State.state_list[0]));
		...
		var offset = (((buttons.members[State.state_list.length-1].x+buttons.members[State.state_list.length-1].width)-buttons.members[0].x)-FlxG.width)/2+50;
